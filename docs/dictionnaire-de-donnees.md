...
| categorie_id          | INT          | Lien vers la catégorie ou sous-catégorie | FK → Catégorie(id)                 |
...
| parent_id   | INT          | Référence à la catégorie parente (null si racine) | FK → Catégorie(id), optionnel  |
| niveau      | INT          | Niveau hiérarchique (ex : 0, 1, 2…)            | ≥ 0, facultatif                    |
...
- **Facturation** : Une facture par commande, même en cas de livraison partielle.
